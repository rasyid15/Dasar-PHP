...
if($_POST){
    ...
    if(empty($nama_kelas)){
        ...
        location.href='tambah_kelas.php';</script>";
    }else if(empty($wali_kelas)){
        echo"<script>alert('wali kelas tidak boleh kosong');
        location.href='tambah_kelas.php';</script>";
    }else{
        include"koneksi.php";
        $insert=mysqli_query($conn,"insert into kelas(nama_kelas, kelompok, wali_kelas) value
        ('".$nama_kelas."','".$kelompok."','".$wali_kelas."')");
        if($insert){
